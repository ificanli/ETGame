...
## 匹配流程
1. 客户端发送匹配请求到Gate
2. Gate转发到Match Scene，并把当前Gate FiberId写入 `GateActorId`
3. Match Scene创建 `MatchRequest` 加入队列，记录玩家所属 Gate
4. 定时器每秒尝试匹配，FIFO先到先得
5. 凑够人数后产出 `MatchResult`，区分真人、机器人补位和目标 Gate 分组
6. Match 统一向 MapManager 创建本局唯一副本，并初始化 Map 侧对局上下文
7. Match 只向相关 Gate 定向下发 `Match2G_MatchSuccess`
8. Gate 只负责通知本地客户端并转发本地真人玩家进图
9. Map 在所有真人进入后本地补位创建机器人
