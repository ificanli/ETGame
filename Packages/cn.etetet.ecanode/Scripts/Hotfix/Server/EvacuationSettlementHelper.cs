...
namespace ET.Server
{
    ...
    public static class EvacuationSettlementHelper
    {
        ...
        {
            Dictionary<int, int> mergedItems = new();
            long totalWealth = 0;
            ...
            if (equipComp != null)
            {
                foreach (var kv in equipComp.EquippedItems)
                {
                    ...
                    AddMergedItem(mergedItems, item.ConfigId, 1);
            ...
            foreach (var kv in mergedItems)
            {
                ItemData itemData = ItemData.Create();
                itemData.ConfigId = kv.Key;
                itemData.Count = kv.Value;
        ...
        private static void AddMergedItem(Dictionary<int, int> mergedItems, int configId, int count)
        {
            ...
            if (mergedItems.TryGetValue(configId, out int current))
            {
                mergedItems[configId] = current + count;
            }
            else
            {
                mergedItems[configId] = count;
            }
        }
    }
}
