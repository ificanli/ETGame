...
namespace ET.Server
{
    ...
    {
        [EntitySystem]
        private static void Awake(this PlayerEvacuationComponent self, long evacuationPointId, long requiredTime, string lobbyMapName)
        {
        ...
        private static bool IsPlayerInRange(this PlayerEvacuationComponent self, Unit player)
        {
            ...
            float distance = ECAHelper.GetHorizontalDistance(player.Position, ecaUnit.Position);
        ...
        private static async ETTask CompleteEvacuation(this PlayerEvacuationComponent self)
        {
            ...
            // 撤离完成，传送到大厅场景（默认 Home）
            player = playerRef;
            await TransferHelper.TransferAtFrameFinish(player, lobbyMapName, 0);
        }
