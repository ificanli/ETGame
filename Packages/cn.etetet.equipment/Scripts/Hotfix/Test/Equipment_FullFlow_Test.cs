...
namespace ET.Test
{
    ...
    public class Equipment_FullFlow_Test : ATestHandler
    {
        public override async ETTask<int> Handle(TestContext context)
        {
            ...
            ItemHelper.AddItem(serverItem, 10004, 2, ItemChangeReason.MonsterDrop);
            ...
            {
                if (item.ConfigId == 10004) hasPickedItem = true;
                if (item.ConfigId == 10001) hasEquipItem = true;
            }

            if (!hasPickedItem)
            {
                Log.Console($"settlement missing picked item ConfigId=10004");
                return 6;
            }
