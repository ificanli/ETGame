...
namespace ET.Client
{
    public class ECAFlowGraphView : GraphView
    {
        ...
        public void LoadGraph()
        {
            ...
            Dictionary<int, ECAFlowNodeView> nodeMap = new();
            foreach (ECAFlowNodeView node in nodes.OfType<ECAFlowNodeView>())
            {
                if (node.NodeId <= 0)
                {
                    Debug.LogError($"[ECAFlowGraph] Graph '{graphAsset?.name}' contains invalid node id: {node.NodeId}, title={node.title}");
                    continue;
                }

                if (!nodeMap.TryAdd(node.NodeId, node))
                {
                    Debug.LogError($"[ECAFlowGraph] Graph '{graphAsset?.name}' contains duplicate node id: {node.NodeId}, title={node.title}");
                }
            }
        ...
        private void ValidateGraph(FlowGraphData data)
        {
            ...
            foreach (FlowNodeData node in data.Nodes)
            {
                ...
                if (!outgoing.TryGetValue(nodeId, out var list))
                {
                    if (node.NodeType != ECAFlowNodeType.Action)
                    {
                        Debug.LogWarning($"[ECAFlowGraph] Node {nodeId}({node.NodeType}) has no outgoing connection.");
                    }
                    continue;
                }
                ...
                if (node.NodeType == ECAFlowNodeType.Condition)
                {
                    ...
                    if (!hasTrue || !hasFalse)
                    {
                        Debug.LogWarning($"[ECAFlowGraph] Condition node {nodeId} missing branch: True={hasTrue}, False={hasFalse}");
                    }
                }
            }
        }
