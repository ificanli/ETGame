...
namespace ET.Client
{
    public class ECAFlowNodeView : Node
    {
        ...
        public ECAFlowNodeView(FlowNodeData data)
        {
            ...
            NodeId = data.NodeId != 0 ? data.NodeId : data.LegacyId;
            NodeKey = data.NodeKey;
            paramsText = BuildParamsText(data.Params);
            title = string.IsNullOrEmpty(data.Title) ? $"{data.NodeType} {NodeId}" : data.Title;
        ...
        private static string GetActionParamTemplate(string nodeKey)
        {
            switch (nodeKey)
            {
                ...
                case ECAFlowActionKey.ApplyStealth:
                    return "self_alpha=0.5";
                case ECAFlowActionKey.RemoveStealth:
                    return string.Empty;
                case ECAFlowActionKey.RefreshDoorInteractHint:
                    return string.Empty;
                case ECAFlowActionKey.ToggleDoor:
                    return string.Empty;
                default:
                    return string.Empty;
            }
