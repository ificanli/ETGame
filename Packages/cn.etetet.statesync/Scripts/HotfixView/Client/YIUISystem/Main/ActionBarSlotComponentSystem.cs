...
namespace ET.Client
{
    public static partial class ActionBarSlotComponentSystem
    {
        ...
        private static void ResetInfo(this ActionBarSlotComponent self)
        {
            ...
            self.u_DataIcon?.SetValue(string.Empty, true);
            self.m_SpellConfig = null;
            self.m_Player = default;
            self.m_Numeric = default;
            self.m_SpellComponent = default;
        ...
        private static bool HasValidDataBindings(this ActionBarSlotComponent self)
        {
            if (self == null || self.IsDisposed)
            {
                return false;
            ...
            bool valid = self.u_DataId != null &&
                self.u_DataHotKey != null &&
                self.u_DataCharge != null &&
                self.u_DataCountDown != null &&
                self.u_DataIconFill != null &&
                self.u_DataIcon != null;
            if (!valid)
            {
                Log.Warning($"[ActionBarSlot] data binding missing: ui={(self.UIBase?.OwnerGameObject != null ? self.UIBase.OwnerGameObject.name : "null")}, id={self.Id}");
            }
