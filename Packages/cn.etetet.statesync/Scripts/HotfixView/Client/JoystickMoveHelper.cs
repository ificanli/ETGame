namespace ET.Client
{
    ...
    /// <summary>
    /// 客户端摇杆移动辅助，供UI层调用。
    /// 优先写入本地 InputSystemComponent，由输入组件统一汇总后发送给服务端。
    /// </summary>
    public static class JoystickMoveHelper
    {
        ...
        public static void SendJoystickInput(Scene root, float dirX, float dirZ)
        {
            ...
            Unit myUnit = currentScene != null ? UnitHelper.GetMyUnitFromCurrentScene(currentScene) : null;
            InputSystemComponent inputSystemComponent = myUnit?.GetComponent<InputSystemComponent>();
            if (inputSystemComponent != null)
            {
                inputSystemComponent.SetJoystickMoveInput(new Vector2(dirX, dirZ), dirX == 0f && dirZ == 0f);
                return;
            }
