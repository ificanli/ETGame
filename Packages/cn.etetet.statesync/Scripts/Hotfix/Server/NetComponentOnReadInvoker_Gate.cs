    ...
    public class NetComponentOnReadInvoker_Gate: AInvokeHandler<NetComponentOnRead>
    {
        ...
        private async ETTask HandleAsync(NetComponentOnRead args)
        {
            ...
            switch (message)
            {
                ...
                {
                    Type requestType = actorLocationRequest.GetType();
                    long unitId = session.GetComponent<SessionPlayerComponent>().Player.Id;
                    int rpcId = actorLocationRequest.RpcId; // 这里要保存客户端的rpcId
                    IResponse iResponse;
                    try
                    {
                        iResponse = await root.GetComponent<MessageLocationSenderComponent>().Get(LocationType.Unit).Call(unitId, actorLocationRequest);
                    }
                    catch (RpcException e)
                    {
                        iResponse = MessageHelper.CreateResponse(requestType, rpcId, e.Error);
                        iResponse.Message = e.Message;
                        Log.Warning($"[Gate] location request rpc failed: unitId={unitId}, request={requestType.FullName}, error={e.Error}");
                    }
                    catch (Exception e)
                    {
                        iResponse = MessageHelper.CreateResponse(requestType, rpcId, ErrorCode.ERR_RpcFail);
                        iResponse.Message = e.ToString();
                        Log.Error(e);
                    }
